...
- export the current PDF with text and whiteout overlays embedded;
...
Not implemented: signatures, initials, checkmarks, images, form filling, undo/redo, page organization, OCR, secure redaction, native editing of existing PDF text, persistence, backend services.
...
2. add text and visual whiteout as overlay elements; Done for current page.
3. export a valid PDF containing those additions; Done for text and whiteout overlays.
...
Live PDF rendering now uses PDF.js behind an infrastructure boundary. The editor renders the active page to a canvas, aligns overlays in CSS page units, and keeps export coordinates independent from zoom and device pixel ratio.
