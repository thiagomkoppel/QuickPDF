...
For a bounded physical-device diagnosis, open `/?pwa-debug=1` while online. After the normal compatibility startup completes, it displays only the launch URL, standalone state, online state, worker controller and registration metadata, shell-cache asset checks, and manifest launch configuration. It never displays document names, document contents, or editor-session data. Record the values before installing and again after the failed offline launch. This temporary diagnostic route should be removed after physical acceptance is complete.
